...
当前项目中 OCR 相关的条件节点（文字检测节点、数字比较节点），在识别到多个目标时，默认从左上往右下识别，返回识别到的第一个预期目标后返回坐标值。本需求旨在为这两个条件节点添加"识别起点"参数，允许用户指定识别的起始方向。
...
NODE_CONFIG_SCHEMAS = {
    "OCRConditionNode": [
        ...
        {"key": "search_direction", "label": "识别起点", "type": "select", "options": ["左上", "右上", "左下", "右下"], "default": "左上"},  # 新增
...
| `doc/02_详细实现方法.md` | 添加识别起点的实现说明 |
| `doc/04_节点功能速查表.md` | 更新 OCR 和数字节点参数列表 |
| `doc/用户使用手册.md` | 添加识别起点功能的使用说明 |
...
识别起点参数用于控制 OCR 相关条件节点在识别到多个目标时，返回哪个目标的位置。
...
| 文件路径 | 修改类型 | 说明 |
|---------|---------|------|
| `bt_utils/direction.py` | **新增** | 识别起点工具模块 |
...
本开发方案针对 OCR 相关条件节点（文字检测、数字比较）设计了识别起点功能，主要结论如下：
