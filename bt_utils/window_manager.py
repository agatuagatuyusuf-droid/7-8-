...
class WindowManager:
    ...
    @staticmethod
    def find_window_smart_with_hwnd(hwnd: int, pid: Optional[int], title_keyword: str,
                                     window_class: str = "") -> Tuple[Optional[int], str]:  # === 枚举弹窗功能
        ...
        # === 枚举弹窗功能 ↓ ===
        if pid and window_class:
            import re
            needle = re.sub(r'^Qt\d+', '', window_class)
            for h in WindowManager.find_all_windows_by_pid(pid):
                buf = ctypes.create_unicode_buffer(256)
                user32.GetClassNameW(h, buf, 256)
                cls = buf.value
                if cls and re.sub(r'^Qt\d+', '', cls).endswith(needle):
                    return h, "class_suffix"
        # === 枚举弹窗功能 ↑ ===
